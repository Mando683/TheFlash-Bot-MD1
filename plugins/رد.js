...
await conn.sendMessage(m.chat, { react: { text: '🥺', key: m.key } })
 ...
     conn.reply(m.chat, `*تــدوم يــاض 😂*`, m) //wm, null, [['Menu', '#menu']], m) botones :V
    ...
    if (/^كسمك$/i.test(m.text) ) { //sem prefixo 
     conn.reply(m.chat, `*امريكا سابت العراق وناكت امك*`, m) //wm, null, [['Menu', '#menu']], m) botones :V
await conn.sendMessage(m.chat, { react: { text: '🗿', key: m.key } })
}

 if (/^يابوت|يا بوت$/i.test(m.text) ) { //sem prefixo 
     conn.reply(m.chat, `*نعم*`, m) //wm, null, [['Menu', '#menu']], m) botones :V 
await conn.sendMessage(m.chat, { react: { text: '🗿', key: m.key } })
 } 
...
await conn.sendMessage(m.chat, { react: { text: '😂', key: m.key } })
   ...
   if (/^بوت خرا|كسم البوت|خرا عليك$/i.test(m.text) ) { //sem prefixo
     conn.reply(m.chat,'  *اخرص يكسمك*',m) //wm, null, [['Menu','#menu']], m) botones :v
 ...
     conn.reply(m.chat, `*شــغــال يعمري*`, m) //wm, null, [['Menu', '#menu']], m) botones :V
